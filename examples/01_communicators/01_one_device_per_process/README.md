# 通信域管理 - 每个进程管理一个 NPU 设备（基于 root 节点信息初始化通信域）

## 样例介绍

本样例展示如何使用 `HcclCommInitRootInfoConfig()` 接口，基于 root 节点信息初始化通信域，包含以下功能点：
...
- 通过 MPI 拉起多个进程，并将 rootinfo 标识信息广播至所有进程。
- 在每个进程中，基于 rootinfo 标识信息通过 `HcclCommInitRootInfoConfig()` 接口初始化通信域。
...
├── main.cc                   # 样例源文件
├── Makefile                  # 编译/构建配置文件
└── one_device_per_process    # 编译生成的可执行文件
```
