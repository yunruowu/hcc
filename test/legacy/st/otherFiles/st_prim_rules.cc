...
protected:
    ...
    void Check_PostReady_WaitFin_PostFinAck(vector<unique_ptr<Instruction>> &result, RankId remote,
                                            const LinkData &link)
    {
    ...
    void Check_PostReady_WaitFin(vector<unique_ptr<Instruction>> &result, RankId remote,
                                            const LinkData &link)
    {
    ...
    void WithoutData_Check_SendGet_Or_RecvPut(vector<unique_ptr<Instruction>> &result, RankId remote,
                                              const LinkData &link)
    {
    ...
    void Check_WaitReady_DMAData_PostFin(vector<unique_ptr<Instruction>> &result, RankId remote,
                                                    const LinkData &link, InstructionType insType, u32 insTypeNum)
    {
    ...
    void Check_WaitReady_DMAData(vector<unique_ptr<Instruction>> &result, RankId remote,
                                                    const LinkData &link, InstructionType insType, u32 insTypeNum)
    {
    ...
    {
        Check_WaitReady_DMAData_WaitFinAck(result, primSend.GetRemoteRank(), primSend.GetLink(),
                                                   InstructionType::WRITE, primSend.Size());
    }

    void Check_RecvGet(vector<unique_ptr<Instruction>> &result, PrimRecv &primRecv)
    {
        Check_WaitReady_DMAData_PostFin_WaitFinAck(result, primRecv.GetRemoteRank(), primRecv.GetLink(),
                                                   InstructionType::READ, primRecv.Size());
    }

    void Check_WithInlineReduce_SendReducePut(vector<unique_ptr<Instruction>> &result, PrimSendReduce &sendReduce)
    {
        Check_WaitReady_DMAData_PostFin_WaitFinAck(result, sendReduce.GetRemoteRank(), sendReduce.GetLink(),
                                                   InstructionType::WRITE_REDUCE, sendReduce.Size());                                                 
    }
    void Check_WithoutInlineReduce_SendReducePut(vector<unique_ptr<Instruction>> &result, PrimSendReduce &sendReduce)
    {
        Check_WaitReady_DMAData_PostFin_WaitFinAck(result, sendReduce.GetRemoteRank(), sendReduce.GetLink(),
                                                   InstructionType::WRITE, sendReduce.Size());
    }
    void Check_WithInlineReduce_RecvReducGet(vector<unique_ptr<Instruction>> &result, PrimRecvReduce &recvReduce)
    {
        Check_WaitReady_DMAData_PostFin_WaitFinAck(result, recvReduce.GetRemoteRank(), recvReduce.GetLink(),
                                                   InstructionType::READ_REDUCE, recvReduce.Size());
    }
    ...
    void Check_WithoutInlineReduce_WithoutPostFinAck_RecvReducePut(vector<unique_ptr<Instruction>> &result, PrimRecvReduce &recvReduce)
    {
        u32 num   = recvReduce.Size();
        u32 total = 2 + num;
        EXPECT_EQ(total, result.size());
                            ...
                            InstructionType insType, u32 insTypeNum)
    {
        ...
        EXPECT_EQ(InstructionType::POST_FIN_ACK, result[4 + insTypeNum]->GetType());
        EXPECT_EQ(InstructionType::WAIT_FIN_ACK, result[5 + insTypeNum]->GetType());
    }
...
TEST_F(PrimRulesTest, translate_send_link_p2p_dma_put_test)
{
    ...
    // then
    Check_WaitReady_DMAData_PostFin(result, primSendDmaPut.GetRemoteRank(), primSendDmaPut.GetLink(),
                                                   InstructionType::WRITE, primSendDmaPut.Size());
}
...
TEST_F(PrimRulesTest, translate_send_link_dev_net_ub_dma_default)
{
    ...
    vector<unique_ptr<Instruction>> result = Translate(primSendDmaDefault);
    // then
    Check_WaitReady_DMAData_PostFin_WaitFinAck(result, primSendDmaDefault.GetRemoteRank(), primSendDmaDefault.GetLink(),
                                                   InstructionType::WRITE, primSendDmaDefault.Size());
}
...
TEST_F(PrimRulesTest, translate_send_link_dev_net_ub_dma_put)
{
    ...
    vector<unique_ptr<Instruction>> result = Translate(primSendDmaPut);
    // then
    Check_WaitReady_DMAData_PostFin_WaitFinAck(result, primSendDmaPut.GetRemoteRank(), primSendDmaPut.GetLink(),
                                                   InstructionType::WRITE, primSendDmaPut.Size());
}
...
TEST_F(PrimRulesTest, translate_recv_link_p2p_dma_default)
{
    ...
    // then
    Check_WaitReady_DMAData_PostFin(result, primRecv.GetRemoteRank(), primRecv.GetLink(),
                                                   InstructionType::READ, primRecv.Size());
}
...
TEST_F(PrimRulesTest, translate_recv_link_p2p_dma_get)
{
    ...
    // then
    Check_WaitReady_DMAData_PostFin(result, primRecvDmaGet.GetRemoteRank(), primRecvDmaGet.GetLink(),
                                                   InstructionType::READ, primRecvDmaGet.Size());
}
...
TEST_F(PrimRulesTest, translate_recv_link_dev_net_rdma_dma_default)
{
    ...
    // then
    WithoutData_Check_SendGet_Or_RecvPut(result, primRecvDmaDefault.GetRemoteRank(), primRecvDmaDefault.GetLink());
}
...
TEST_F(PrimRulesTest, translate_recv_link_dev_net_rdma_dma_put)
{
    ...
    vector<unique_ptr<Instruction>> result = Translate(primRecvDmaPut);
    // then
    WithoutData_Check_SendGet_Or_RecvPut(result, primRecvDmaPut.GetRemoteRank(), primRecvDmaPut.GetLink());
}
...
TEST_F(PrimRulesTest, translate_send_reduce_link_p2p_fp32_sum_support_inline_reduce_dma_default_and_dma_get)
{
    ...
    PrimSendReduce primSendReduce(remoteRank, link, localSlice, remoteSrcSlice, remoteDstSlice, DataType::FP32,
                                  ReduceOp::SUM);
    ...
    const PrimSendReduce primSendReduceDmaGet(remoteRank, link, localSlice, remoteSrcSlice, remoteDstSlice,
                                              DataType::FP32, ReduceOp::SUM, DmaMode::GET);
    // When
...
TEST_F(PrimRulesTest, translate_send_reduce_link_p2p_uint64_prod_not_support_inline_reduce_dma_default_and_dma_get)
{
    ...
    PrimSendReduce primSendReduce(remoteRank, link, localSlice, remoteSrcSlice, remoteDstSlice, DataType::UINT64,
                                  ReduceOp::PROD);
    PrimSendReduce primSendReduceDmaGet(remoteRank, link, localSlice, remoteSrcSlice, remoteDstSlice, DataType::UINT64,
                                        ReduceOp::PROD, DmaMode::GET);
    // When
    vector<unique_ptr<Instruction>> result1 = Translate(primSendReduce);
    vector<unique_ptr<Instruction>> result2 = Translate(primSendReduceDmaGet);
    // then
...
TEST_F(PrimRulesTest, translate_send_reduce_link_dev_net_rdma_uint64_prod_not_support_inline_reduce_dma_default)
{
    ...
    // then
    Check_WithoutInlineReduce_SendReducePut(result1, primSendReduce);
}
...
TEST_F(PrimRulesTest, translate_recv_reduce_link_p2p_fp32_sum_support_inline_reduce_dma_default_and_dma_get)
{
    ...
    // 910A support FP32+ReduceOp::SUM inline reduce
    PrimRecvReduce primRecvReduce(remoteRank, link, localSlice, remoteSrcSlice, remoteDstSlice, DataType::FP32,
                                  ReduceOp::SUM);
    ...
    PrimRecvReduce primRecvReduceDmaGet(remoteRank, link, localSlice, remoteSrcSlice, remoteDstSlice, DataType::FP16,
                                        ReduceOp::SUM, DmaMode::GET);
    // When
    ...
    // then
    Check_WithoutInlineReduce_RecvReduceGet(result2, primRecvReduceDmaGet);
    Check_WaitReady_DMAData_PostFin(result1, primRecvReduce.GetRemoteRank(), primRecvReduce.GetLink(),
                                    InstructionType::READ_REDUCE, primRecvReduce.Size());
}
       ...
       translate_recv_reduce_link_p2p_uint64_prod_not_support_inline_reduce_dma_default_and_dma_get_one_slice)
{
    ...
    PrimRecvReduce primRecvReduce(remoteRank, link, localSlice, remoteSrcSlice, remoteDstSlice, DataType::UINT64,
                                  ReduceOp::PROD);
    // When
    vector<unique_ptr<Instruction>> result1 = Translate(primRecvReduce);
...
TEST_F(PrimRulesTest, translate_recv_reduce_link_dev_net_rdma_fp32_sum_support_inline_reduce_dma_default)
{
    ...
    // then
    Check_WithoutInlineReduce_RecvReducePut(result1, primRecvReduce);
}
