...
namespace Hccl {
static CcuInstRegister<CcuContextAllReduceMesh2DTwoShot> g_registerCcuAllReduce2DTwoShot(
    CcuInstType::CCU_ALL_REDUCE_MESH_2D_TWO_SHOT_DIRECT);

CcuTempAllReduceMesh2DTwoShot::CcuTempAllReduceMesh2DTwoShot(const RankId virtualRank, const u32 tempRankSize,
    ...
    : CcuAlgTemplateBase(virtualRank, tempRankSize, tempVTopo, tempVirtRankMap)
{
    ...
    if (tempVTopo_.size() != 2 || tempVTopo_[0].size() <= 1 || tempVTopo_[1].size() <= 1) { // concurrmesh的topoMatch返回的vTopo大小应当为2，对应X轴和Y轴的大小
        THROW<InvalidParamsException>(StringFormat("[CcuTempAllReduceMesh2DTwoShot] Rank[%d], Invalid tempVTopo "
...
HcclResult CcuTempAllReduceMesh2DTwoShot::CalcSliceInfo(const AllignInfo &allignInfo, const u64 dataSize,
...
HcclResult CcuTempAllReduceMesh2DTwoShot::CalcRes(AlgTempResReq &tempResReq)
{
    ...
    for (u32 dim = 0; dim < tempVTopo_.size(); dim++) {
        ...
        for (u32 queIdx = 0; queIdx < tempVTopo_[dim].size() - 1; queIdx++) {
            ...
            HCCL_INFO("[CcuTempAllReduceMesh2DTwoShot] Rank[%d], Dim[%u], NeighborRank[%d].", myRank_, dim,
                       neighborRank);
    ...
    for (auto pair : tempLinks) {
        if (pair.second.size() == 0 || pair.second[0].GetHop() != 1) { // ESL环境上暂只有直连链路
            THROW<InvalidParamsException>(
                StringFormat("[CcuTempAllReduceMesh2DTwoShot] Rank[%d]--Peer[%d], InvalidHop[%u].", myRank_, pair.first,
                             pair.second[0].GetHop()));
        }
        if ((pair.first / dimSize_[0] == myRank_ / dimSize_[0]) && pair.second[0].GetHop() == 1) {
            HCCL_INFO("[CcuTempAllReduceMesh2DTwoShot][Run] Rank[%d] insert link to Rank[%d] in linksX", myRank_,
                       pair.first);
            linksX_.emplace_back(pair.second[0]);
        } else if ((pair.first % dimSize_[0] == myRank_ % dimSize_[0]) && pair.second[0].GetHop() == 1) {
            HCCL_INFO("[CcuTempAllReduceMesh2DTwoShot][Run] Rank[%d] insert link to Rank[%d] in linksY", myRank_,
                       pair.first);
            linksY_.emplace_back(pair.second[0]);
        } else {
            THROW<InvalidParamsException>(
                StringFormat("[CcuTempAllReduceMesh2DTwoShot] Rank[%d], Unexpected peerRank[%d] in tempLinks.", myRank_,
                             pair.first));
        }
    }
    HCCL_INFO("[CcuTempAllReduceMesh2DTwoShot] PrepareLinks Eends. linksX Size[%u], linksY Size[%u]",
...
HcclResult CcuTempAllReduceMesh2DTwoShot::Run(const TempFuncs &tempFuncs, const RankSliceInfo &sliceInfoVec,
                                              ...
                                              std::vector<InsQuePtr> &tempInsQues)
{
    ...
    HCCL_INFO("[Run] lastRankDataSize[%llu] lastRankXSliceSize[%llu], lastRankYSliceSize[%llu]",
    ...
    for (uint32_t axisId = 0; axisId < 2; axisId++) {  // 2D算法，需要下发 2 条通信指令
        ...
        ccuInstruction.Init(dimSize_, myRank_, inputAddr, outputAddr, axisId, normalRankXSliceSize, normalRankYSliceSize,
