...
static CcuInstRegister<CcuContextReduceScatterMeshMem2Mem2D> g_registrarReduceScatterMeshMem2Mem2D(
    CcuInstType::CCU_REDUCE_SCATTER_MESH_2D_MEM2MEM);

CcuTempReduceScatterMeshMem2Mem2D::CcuTempReduceScatterMeshMem2Mem2D(const RankId virtualRank, const u32 tempRankSize,
    ...
    : CcuAlgTemplateBase(virtualRank, tempRankSize, tempVTopo, tempVirtRankMap)
{
    if (tempVTopo_.size() != 2 || tempVTopo_[0].size() <= 1 || tempVTopo_[1].size() <= 1) { // concurrmesh的topoMatch返回的vTopo大小应当为2，对应X轴和Y轴的大小
        THROW<InvalidParamsException>(StringFormat("[CcuTempReducescatterMeshMem2Mem2D] Rank[%d], Invalid tempVTopo "
...
CcuTempReduceScatterMeshMem2Mem2D::~CcuTempReduceScatterMeshMem2Mem2D()
{
}

void CcuTempReduceScatterMeshMem2Mem2D::InitReduceInfo(const ReduceOp &reduceOp, const DataType &dataType) {
    reduceOp_ = reduceOp;
    dataType_ = dataType;
}

HcclResult CcuTempReduceScatterMeshMem2Mem2D::CalcSliceInfo(const AllignInfo &allignInfo, const u64 dataSize,
...
HcclResult CcuTempReduceScatterMeshMem2Mem2D::CalcRes(AlgTempResReq &tempResReq)
{
    ...
    if (dieNum != 2) {  // concurrmesh的topoMatch返回的vTopo大小应当为2，对应X轴和Y轴的大小
        HCCL_ERROR("[CcuTempReduceScatterMeshMem2Mem2D] Rank[%d], Invalid IODieNum[%zu].", myRank_, tempVTopo_.size());
        return HcclResult::HCCL_E_PARA;
    }
    HCCL_INFO("[CcuTempReduceScatterMeshMem2Mem2D] Rank[%d] requiredQueNum[%u] VtopoSize[%u], VtopoSize0[%u] VtopoSize1[%u].",
    ...
    for (u32 dim = 0; dim < tempVTopo_.size(); dim++) {
        ...
        for (u32 queIdx = 0; queIdx < tempVTopo_[dim].size() - 1; queIdx++) {
            ...
            HCCL_INFO("[CollAlgFactory] [CcuTempReducescatterMeshMem2Mem2D] Rank[%d], Dim[%u], NeighborRank[%d].", myRank_,
                       dim, neighborRank);
...
HcclResult CcuTempReduceScatterMeshMem2Mem2D::Run(const TempFuncs &tempFuncs, const RankSliceInfo &sliceInfoVec,
                                          	  ...
                                          	  std::vector<InsQuePtr> &tempInsQues)
{
    ...
    for (auto &pair : tempLinks) {
        if (pair.second.size() == 0 || pair.second[0].GetHop() != 1) {
            THROW<InvalidParamsException>(StringFormat("[CcuTempAlltoAllMeshMem2Mem2D] Rank[%d]--Peer[%d], InvalidHop[%u].",
                myRank_, pair.first, pair.second[0].GetHop()));
        }
        if ((pair.first / dimSize_[0] == myRank_ / dimSize_[0]) && pair.second[0].GetHop() == 1) {
            HCCL_INFO("[CcuTempReduceScatterMeshMem2Mem2D][Run] Rank[%d] insert link to Rank[%d] in linksX", myRank_, pair.first);
            linksX_.emplace_back(pair.second[0]);
        } else if ((pair.first % dimSize_[0] == myRank_ % dimSize_[0]) && pair.second[0].GetHop() == 1) {
            HCCL_INFO("[CcuTempReduceScatterMeshMem2Mem2D][Run] Rank[%d] insert link to Rank[%d] in linksY", myRank_, pair.first);
            linksY_.emplace_back(pair.second[0]);
        } else {
            THROW<InvalidParamsException>(StringFormat(
                "[CcuTempAlltoAllMeshMem2Mem2D] Rank[%d], Unexpected peerRank[%d] in tempLinks.", myRank_, pair.first));
        }
    }
    ...
    for (uint32_t axisId = 0; axisId < 2; axisId++) {  // 2D算法需要执行两次
        // 计算每次编译的偏移量和数据量
        ...
        CcuInstructionReduceScatterMeshMem2Mem2D ccuInsReduceScatterMeshMem2Mem2D;

        ccuInsReduceScatterMeshMem2Mem2D.Init(dimSize_, static_cast<uint32_t>(myRank_), inputAddr, outputAddr, axisId,
					      outputSize, xAxisSize, yAxisSize, offSet, token, op_, tempVTopo_);

        HCCL_INFO("[CcuTempReduceScatterMeshMem2Mem2D] Init: dimSize0[%llu], dimSize1[%llu], myRank_[%d], inputAddr[%llu],"\
        ...
        ccuInsReduceScatterMeshMem2Mem2D.SetLinks(axisId == 0 ? linksX_ : linksY_);
        ccuInsReduceScatterMeshMem2Mem2D.SetRankGroup(axisId == 0 ? rankGroupX : rankGroupY);
        u32 ckeNum = 5;
        ccuInsReduceScatterMeshMem2Mem2D.SetCntCkeNum(ckeNum);  // 每个transport使用4个CKE
        ccuInsReduceScatterMeshMem2Mem2D.Describe();
        insGroupPtr->Append(std::move(std::make_unique<CcuInstructionReduceScatterMeshMem2Mem2D>(ccuInsReduceScatterMeshMem2Mem2D)));
    }
