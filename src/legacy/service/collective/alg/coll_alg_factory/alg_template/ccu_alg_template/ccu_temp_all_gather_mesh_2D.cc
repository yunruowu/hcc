...
CcuTempAllGatherMesh2D::CcuTempAllGatherMesh2D(const RankId virtualRank, const u32 tempRankSize,
    ...
    : CcuAlgTemplateBase(virtualRank, tempRankSize, tempVTopo, tempVirtRankMap)
{
    ...
    if (tempVTopo_.size() != 2 || tempVTopo_[0].size() <= 1 || tempVTopo_[1].size() <= 1) { // concurrmesh的topoMatch返回的vTopo大小应当为2，对应X轴和Y轴的大小
        THROW<InvalidParamsException>(StringFormat("[CcuTempAllGatherMesh2D] Rank[%d], Invalid tempVTopo "
...
HcclResult CcuTempAllGatherMesh2D::CalcRes(AlgTempResReq &tempResReq)
{
    ...
    if (dieNum != 2) {  // concurrmesh的topoMatch返回的vTopo大小应当为2，对应X轴和Y轴的大小
        THROW<InvalidParamsException>(StringFormat("[CcuTempAllGatherMesh2D] Rank[%d], Invalid IODieNum[%u].",
            myRank_, dieNum));
    }
    HCCL_INFO("[CcuTempAllGatherMesh2D] Rank[%d] requiredQueNum[%u] VtopoSize[%u], VtopoSize0[%u] VtopoSize1[%u].",
    ...
    for (u32 dim = 0; dim < tempVTopo_.size(); dim++) {
        ...
        for (u32 queIdx = 0; queIdx < tempVTopo_[dim].size() - 1; queIdx++) {
            ...
            HCCL_INFO("[CollAlgFactory] [CcuTempAllGatherMesh2D] Rank[%d], Dim[%u], NeighborRank[%d].", myRank_,
                       dim, neighborRank);
...
*/
HcclResult CcuTempAllGatherMesh2D::CalcSliceInfo(const AllignInfo &allignInfo, const u64 dataSize,
...
HcclResult CcuTempAllGatherMesh2D::Run(const TempFuncs &tempFuncs, const RankSliceInfo &sliceInfoVec, const BuffInfo &buffInfo,
                         const ResLinks &tempLinks, std::vector<InsQuePtr> &tempInsQues)
{
    ...
    for (auto pair : tempLinks) {
        if (pair.second.size() == 0 || pair.second[0].GetHop() != 1) {  // ESL环境上暂只有直连链路
            THROW<InvalidParamsException>(StringFormat("[CcuTempAllGatherMesh2D] Rank[%d]--Peer[%d], InvalidHop[%u].",
    ...
    if (opMode_ == OpMode::OPBASE) {
        ...
        outputAddr = BufferTypeToAddr(buffInfo_.outBuffType) + buffInfo_.outBuffBaseOff + tempFuncs.usrData.usrInSlices[0].GetOffset();
    }
    ...
    HCCL_INFO("[CcuTempAllGatherMesh2D] Rank[%d], input[%llu], output[%llu].",
        myRank_, inputAddr, outputAddr);
    ...
    for (uint32_t axisId = 0; axisId < 2; axisId++) {  // 2D算法，需要执行两次
        HCCL_INFO("[CcuTempAllGatherMesh2D][Run] Rank[%d], axisId[%u], aSize[%llu], bSize[%llu].",
