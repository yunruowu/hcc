...
#ifndef HCCLV2_CCU_INSTRUCTION_ALL_GATHER_NHR_1D_H_
#define HCCLV2_CCU_INSTRUCTION_ALL_GATHER_NHR_1D_H_
...
// 为AllGatherNHR1D实现的CCUIns、CCUCtxArg与CCUTaskArg
class CcuCtxArgAllGatherNHR1D : public CcuCtxArg {
public:
    explicit CcuCtxArgAllGatherNHR1D(const std::vector<uint64_t> &dimSize, uint32_t rankId, uint32_t axisId, uint32_t axisSize,
    ...
    CcuCtxSignature GetCtxSignature() const override
    {
        ...
        GenerateCcuCtxSignature(signature, CcuInstType::CCU_ALLGATHER_NHR_1D_MEM2MEM, op_, tempVTopo_);
        return signature;
    }
...
class CcuTaskArgAllGatherNHR1D : public CcuTaskArg {
public:
...
class CcuInstructionAllGatherNHR1D : public CcuInstruction {
public:
    CcuInstructionAllGatherNHR1D() : CcuInstruction()
    {
    }

    void Init(uint32_t rankId, uint64_t inputAddr, uint64_t outputAddr, uint64_t axisId, uint32_t axisSize,
              uint64_t die0Size, uint64_t die1Size, uint64_t repeatNum, uint64_t inputSliceStride,
              uint64_t outputSliceStride, uint64_t inputRepeatStride, uint64_t outputRepeatStride,
              std::vector<NHRStepInfo> stepInfoVector, std::map<u32, u32> indexMap, uint64_t token,
              uint64_t isInputOutputEqual, CollAlgOperator &op, std::vector<std::vector<RankId>> &tempVTopo)
    {
        u32 maxDimNum = 1;
        if (tempVTopo.size() != maxDimNum) {
            THROW<InvalidParamsException>(StringFormat(
                "[CcuInstructionAllGatherNHR1D] tempVTopo size is not 1, size is [%zu].", tempVTopo.size()));
        }
    ...
    {
        return StringFormat("CcuInstructionAllGatherNHR1D rankId [%u], instType[%s]", rankId_,
    ...
    {
        return std::make_unique<CcuCtxArgAllGatherNHR1D>(dimSize_, rankId_, axisId_, axisSize_, stepInfoVector_,
                                                         indexMap_, op_, tempVTopo_);
    }

    std::unique_ptr<CcuTaskArg> GetTaskArg() const override
    {
        return std::make_unique<CcuTaskArgAllGatherNHR1D>(inputAddr_, outputAddr_, token_, die0Size_, die1Size_,
                                                          repeatNum_, inputSliceStride_, outputSliceStride_,
                                                          inputRepeatStride_, outputRepeatStride_, isInputOutputEqual_);
    }
...
private:
    CcuInstType                      instType_ = CcuInstType::CCU_ALLGATHER_NHR_1D_MEM2MEM;
    std::vector<uint64_t>            dimSize_;
    uint32_t                         rankId_{0};
    uint32_t                         axisId_{0};
...
#endif // HCCLV2_CCU_INSTRUCTION_ALL_GATHER_NHR_1D_H_
