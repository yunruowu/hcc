...
#ifndef HCCLV2_CCU_INSTRUCTION_ALL_REDUCE_MESH_1D_H_
#define HCCLV2_CCU_INSTRUCTION_ALL_REDUCE_MESH_1D_H_
...
public:
    ...
    CcuCtxSignature GetCtxSignature() const override
    {
        ...
        GenerateCcuCtxSignature(signature, CcuInstType::CCU_ALL_REDUCE_MESH_1D_DIRECT, op_, tempVTopo_);
        return signature;
    }
        ...
        uint64_t token, CollAlgOperator &op, std::vector<std::vector<RankId>> &tempVTopo)
    {
        ...
        if (tempVTopo.size() != maxDimNum) {
            THROW<InvalidParamsException>(StringFormat(
                "[CcuInstructionAllReduceMesh1D] tempVTopo size is not 1, size is [%zu].", tempVTopo.size()));
        }
