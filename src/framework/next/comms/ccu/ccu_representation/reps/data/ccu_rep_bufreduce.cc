...
CcuRepBufReduce::CcuRepBufReduce(const std::vector<CcuBuf> &mem, uint16_t count, uint16_t dataType,
...
bool CcuRepBufReduce::Translate(CcuInstr *&instr, uint16_t &instrId, const TransDep &dep)
{
    ...
    if (count > CCU_REDUCE_MAX_MS || mem.size() > CCU_REDUCE_MAX_MS) {
        Hccl::THROW<Hccl::CcuApiException>("count and mem size must less than %u", CCU_REDUCE_MAX_MS);
    }
