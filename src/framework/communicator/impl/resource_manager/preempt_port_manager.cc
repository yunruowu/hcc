    ...
    for (auto &range: portRange) {
        for (u32 port = range.min; port <= range.max; ++port) {
            ...
            CHK_PRT_RET(ret != HCCL_E_UNAVAIL,
                HCCL_ERROR("[PreemptPortManager][PreemptPortInRange] attempt to listen on port[%u] for ip[%s] fail."
                " some unexpected error occurs, errNo[0x%016llx]. attemptation is stopped.",
                port, ipAddr.c_str(), ret), ret);
            // 当前端口已被占用，尝试抢占下一个端口
            HCCL_INFO("[PreemptPortManager][PreemptPortInRange] could not listen on ip[%s], port[%u].",
                ipAddr.c_str(), port);
        }
    }
