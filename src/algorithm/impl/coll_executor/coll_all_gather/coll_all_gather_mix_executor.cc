...
HcclResult CollAllGatherMixExecutor::KernelRun(const OpParam &param, ExecMem &execMem)
{
    ...
    if (!DMAReduceFlag_) {
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[CollAllGatherMixExecutor][KernelRun]AllGather mix memcpy Failed, Offset[%llu], Size[%llu]",
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[CollAllGatherMixExecutor][KernelRun]AllGather mix user memcpy "
                "Failed, Offset[%llu], Size[%llu]", baseOffset + level0Offset, inputMemSize), ret);
    }

    //  第一步，AI server间AllGather
    ...
    HCCL_INFO("[CollAllGatherMixExecutor][KernelRun]AllGather mix run success");
    return HCCL_SUCCESS;
}
