...
#include "all_gather_v_pipeline.h"
...
HcclResult AllGatherVPipeline::MainWaitSub()
{
...
HcclResult AllGatherVPipeline::SubRecordMain()
{
    u32 subStreamNum = intraRankSize_;
...
HcclResult AllGatherVPipeline::MainRecordSub()
{
    u32 subStreamNum = intraRankSize_;
    for (u32 signalIndex = 0; signalIndex < subStreamNum; signalIndex++) {
...
HcclResult AllGatherVPipeline::SubWaitMain()
{
    u32 subStreamNum = intraRankSize_;
    for (u32 streamIndex = 0; streamIndex < subStreamNum; streamIndex++) {
...
HcclResult AllGatherVPipeline::RunAsync()
{
    HCCL_INFO("[AllGatherVPipeline][RunAsync]AllGatherRingMesh starts groupRankId[%u] ", userRank_);
    // inter ring algo
    ...
    HCCL_INFO("[AllGatherVPipeline][RunAsync]AllGatherRingMesh finished groupRankId[%u] ", userRank_);
    return HCCL_SUCCESS;
}
...
REGISTER_TEMPLATE(TemplateType::TEMPLATE_ALL_GATHER_V_PIPELINE, AllGatherVPipeline);
} // namespace hccl