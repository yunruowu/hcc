...
HcclResult AllGatherMesh::Rx(const LINK &link, const Slice &srcSlice, const Slice &rxSlice, Stream stream)
{
    ...
    HCCL_DEBUG("rank[%u] rx rcvMem[%p] output's offset[%llu] size[%llu] rcv data from srcrank's"
        "offset[%llu] ",
    ...
    const std::vector<Slice> &inputSlices)
{
    ...
    for (u32 round = 1; round < interRankSize_; round++) {
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS,
            HCCL_ERROR("[Run][AllGather]destrank[%u] AllGather mesh run tempAlg barrier "
            "failed",
            dstRank),
            ret);
